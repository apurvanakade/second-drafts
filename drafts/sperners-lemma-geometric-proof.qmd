...
```{ojs}
// Sinusoidal easing function mapping [0,1] → [0,1]
...
buildColorMap2 = N => {
  ...
  const baryCoords = generateBarycentricCoords(N)
  baryCoords.forEach(([a, b, c]) => {
    map.set(`${a},${b},${c}`, assignColor2(a, b, c))
  })
  return map
}

colorMap2 = buildColorMap2(N)
```
...
```{ojs}
morphedColoredPointsMap2 = interpolatePoints(s, barycentricCoords, equilateralTriangleVertices, colorMap2)
```

```{ojs}
subTriangles2 = generateSubTriangles(N, morphedColoredPointsMap2.morphedMap)
```


```{ojs}
viewof plotRGB2 = Plot.plot({
  width: 800,
  height: 800,
  x: {label: "x", domain: [0, 1]},
  y: {label: "y", domain: [0, Math.sqrt(3) / 2]},
  marks: [
    ...
    Plot.geo({
      type: "FeatureCollection",
      features: subTriangles2.map(tri => createFeature(tri, morphedColoredPointsMap2.morphedMap))
    }, {
      fill: d => d.properties.fillColor,
      stroke: "black",
      strokeWidth: 1
    }),
    Plot.dot(morphedColoredPointsMap2.morphedPoints, {
      x: "x",
      y: "y",
      fill: d => d.color,
      r: 5 * (5 / N)
