...
This picture is the proof of [Sperner's Lemma](./9994-sperners-lemma-part-01.qmd).  
...
1. <span style="background-color:rgba(0, 0, 0, 0.25)">**RGB**</span> triangular rooms have **only one** door.
2. <span style="background-color:rgba(255, 215, 0, 1)">**RGG**</span> and <span style="background-color:rgba(255, 215, 0, 1)">**RRG**</span> triangular rooms have **two** doors.
3. All other triangular rooms have **no** doors.

Thus, we already have a way to quantify <span style="background-color:rgba(0, 0, 0, 0.25)">**RGB**</span> triangles — they are exactly the rooms with an **odd** number of doors. So, we can restate Sperner's Lemma as saying:
...
The <span style="background-color:rgba(255, 215, 0, 1)">yellow</span> rooms have a path going across them from one `RG` door to another, while the <span style="background-color:rgba(0, 0, 0, 0.25)">**RGB**</span> rooms have only one door to both enter and exit.

Here are a few things you can immediately deduce using pure logic:

1. No two paths can intersect.
2. Some paths are loops with no endpoints.
3. The paths that are not loops have exactly **two endpoints**. Hence, the total number of endpoints is **even**.
4. Each endpoint can be exactly one of the following:  
   a. An `RG` door on the boundary of the original triangle.  
   b. An <span style="background-color:rgba(0, 0, 0, 0.25)">**RGB**</span> triangular room.
...
- the number of <span style="background-color:rgba(0, 0, 0, 0.25)">**RGB**</span> triangles  
