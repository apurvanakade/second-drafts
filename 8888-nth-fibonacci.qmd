...
I'm not certain this is true, but it holds for all integers up to 10,000. I'm also interested in understanding the distribution of $n / \alpha(n)$. When plotting $\alpha(n) / n$, most values fall below 1, though some are greater than or equal to 1.
...
where $\phi = \frac{1 + \sqrt{5}}{2}$ and $\psi = \frac{1 - \sqrt{5}}{2}$, to find a closed form for $\alpha(n)$? We know in addition that $\mathbb{Z}[\phi]$ is a unique factorization domain, so we can factor $n$ in terms of $\phi$ and $\psi$.
